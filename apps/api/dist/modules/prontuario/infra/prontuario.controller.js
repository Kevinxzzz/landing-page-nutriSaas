...
// Evolução Clínica
export async function listarEvolucoes(req, res) {
...
export async function criarEvolucao(req, res) {
    const { pacienteId } = req.params;
...
export async function editarEvolucao(req, res) {
    const { id } = req.params;
...
export async function criarMedida(req, res) {
    const { pacienteId } = req.params;
    const paciente = await prisma.paciente.findUnique({ where: { id: pacienteId } });
    if (!paciente)
        throw new AppError('Paciente não encontrado', 404);
...
export async function excluirMedida(req, res) {
    const { id } = req.params;
    const existente = await prisma.medida.findUnique({ where: { id } });
    if (!existente)
        throw new AppError('Medida não encontrada', 404);
