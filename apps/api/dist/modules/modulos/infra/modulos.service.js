...
export async function criarPergunta(clinicaId, moduloId, data) {
    ...
    const alvo = await resolverQuestionarioParaMutacaoEstrutural(clinicaId, modulo.questionarioId);
    ...
    if (alvo.versionado) {
        ...
        if (!moduloClonado) {
            throw new AppError("Módulo correspondente não encontrado na nova versão", 404);
        }
...
export async function editarModulo(clinicaId, moduloId, data) {
    const modulo = await prisma.modulo.findFirst({
        where: {
            id: moduloId,
            questionario: {
    ...
    const novaOrdem = data.ordem === undefined
        ? modulo.ordem
...
export async function excluirModulo(clinicaId, moduloId) {
    const modulo = await prisma.modulo.findFirst({
        where: {
            id: moduloId,
            questionario: {
...
export async function editarPergunta(clinicaId, moduloId, perguntaId, data) {
...
export async function excluirPergunta(clinicaId, moduloId, perguntaId) {
    const pergunta = await prisma.pergunta.findFirst({
        where: {
            id: perguntaId,
            moduloId,
    ...
    const alvo = await resolverQuestionarioParaMutacaoEstrutural(clinicaId, pergunta.modulo.questionarioId);
    ...
    if (alvo.versionado) {
        ...
        if (!perguntaClonada) {
            throw new AppError("Pergunta correspondente não encontrada na nova versão", 404);
        }
        moduloAlvoId = moduloClonado.id;
