...
const REGRAS_PADRAO = [
    {
        pontuacaoMin: 0,
        pontuacaoMax: 30,
        classificacao: "Pessoa mais saudável, menores chances de hipersensibilidades",
    },
    {
        pontuacaoMin: 31,
        pontuacaoMax: 39,
    ...
    },
    {
        pontuacaoMin: 100,
        pontuacaoMax: 9999,
        classificacao: "Saúde muito ruim — alta dificuldade para tarefas diárias, possível associação com doenças crônicas e degenerativas",
    },
];
...
async function upsertLabelsEscolhaPadrao(questionarioId) {
    await Promise.all(Object.entries(LABELS_ESCOLHA_PADRAO).map(([nota, label]) => prisma.labelEscolha.upsert({
        where: {
            questionarioId_nota: {
                questionarioId,
                nota: Number(nota),
...
export async function criarQuestionarioPadraoParaClinica(clinicaId) {
