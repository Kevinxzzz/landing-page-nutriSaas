...
function mapLabelEscolha(item: { id: string; nota: number; label: string }) {
  return {
    id: item.id,
    nota: item.nota,
    label: item.label,
...
async function buscarQuestionarioDaClinica(
  clinicaId: string,
  questionarioId: string,
) {
...
async function clonarQuestionarioParaNovaVersao(
  clinicaId: string,
  questionarioId: string,
) {
...
export async function resolverQuestionarioParaMutacaoEstrutural(
  clinicaId: string,
  questionarioId: string,
): Promise<VersionamentoEstruturalResultado> {
  const questionario = await prisma.questionario.findFirst({
    where: { id: questionarioId, clinicaId },
    select: {
      id: true,
  ...
  const novoQuestionario = await clonarQuestionarioParaNovaVersao(
    clinicaId,
    questionarioId,
  );
...
export async function criar(clinicaId: string, data: CreateQuestionarioInput) {
...
export async function listar(
  clinicaId: string,
  params: ListarQuestionariosParams,
) {
  const where = {
    clinicaId,
...
export async function listarModulosComPerguntas(
  questionarioId: string,
  params: ListarQuestionarioCompletoParams,
) {
  const questionario = await prisma.questionario.findFirst({
    where: {
      id: questionarioId,
...
export async function upsertTitulosEscolha(
  clinicaId: string,
  questionarioId: string,
  itens: LabelEscolhaItemInput[],
) {
  await buscarQuestionarioDaClinica(clinicaId, questionarioId);

  const titulos = await prisma.$transaction(
    itens.map((item) =>
...
export async function excluir(clinicaId: string, questionarioId: string) {
  const existente = await prisma.questionario.findFirst({
    where: { id: questionarioId, clinicaId },
...
export async function adicionarModulo(
  clinicaId: string,
  questionarioId: string,
  data: CreateQuestionarioModuloInput,
) {
...
export async function adicionarRegra(
  clinicaId: string,
  questionarioId: string,
  data: CreateQuestionarioRegrasInterpretacaoInput,
) {
  const alvo = await resolverQuestionarioParaMutacaoEstrutural(
    clinicaId,
    questionarioId,
  ...
  if (data.pontuacaoMin > data.pontuacaoMax) {
    throw new AppError(
      "pontuacaoMin deve ser menor ou igual à pontuacaoMax",
      400,
    );
  }
  ...
  if (faixaSobreposta) {
    throw new AppError(
      "Faixa de pontuação sobreposta com regra existente",
      400,
    );
  }
...
export async function criarQuestionarioDefault(clinicaId: string) {
